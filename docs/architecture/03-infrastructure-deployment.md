...
|---|---|---|
| `nginx-gateway` | Reverse proxy, TLS termination, routing theo path prefix | Có — 80/443 |
...
- Chỉ `nginx-gateway` (và `kafka-ui` khi dev) expose port ra ngoài; `sqlserver`, `redis`, `kafka`, 5 service .NET chỉ giao tiếp nội bộ qua tên container (xem [service discovery](01-overview.md#4-service-discovery-ở-mức-docker-compose)).
...
2. **On merge main**: build & test lại → `docker build` 5 image, tag `git-sha` + `latest` → push GitHub Container Registry (ghcr.io, miễn phí) → SSH vào VPS (vd. `appleboy/ssh-action`) chạy `docker compose pull && docker compose up -d`.
