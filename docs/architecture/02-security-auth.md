...
- Auth Service expose `GET /.well-known/jwks.json`. Chỉ **API Gateway** cấu hình JWT Bearer middleware trỏ vào endpoint này (tự cache và refresh JWKS định kỳ) — các service phía sau (Auth Service, Market Price Service, Marketplace Service, AI Advisory Service) không tự verify JWT nữa, xem mục 2.
...
Điểm quan trọng: **API Gateway verify token tập trung, stateless** (RS256, JWKS cache cục bộ), không gọi Auth Service theo từng request — chỉ middleware JWKS ở Gateway mới cần fetch lại theo chu kỳ (vài giờ/lần, hoặc khi gặp `kid` không khớp cache). Sau khi verify, Gateway gắn danh tính người dùng vào header nội bộ (`X-User-Id`, `X-User-Role`, `X-User-Phone`) rồi mới forward — service phía sau (Auth Service, Market Price Service, Marketplace Service, AI Advisory Service) đọc các header này qua `AddTrustedHeaderAuthentication` (`HappyFarmer.Shared.Contracts/Auth/TrustedHeaderAuthenticationHandler.cs`), **không tự verify chữ ký JWT nữa** (Notification Service chưa áp dụng — vẫn là skeleton rỗng). Model này chỉ an toàn vì mạng Docker nội bộ không public, chỉ Gateway mới có đường vào từ ngoài (xem [01-overview.md](01-overview.md#2-networking) — chỉ container gateway expose port ra host); ở local dev do các service vẫn bind port ra host nên về lý thuyết bị bypass được nếu gọi thẳng port và tự gắn header — trade-off chấp nhận ở dev, xem `CLAUDE.md`.
...
- **Secrets**: `.env` (gitignored) cho local, GitHub Actions Secrets cho CI/CD. Không commit API key (Gemini, OpenWeatherMap, Cloudinary) vào repo — chỉ commit `.env.example`.
