...
**Trạng thái hiện tại**: Auth Service và Market Price Service đã code xong đầy đủ (API + DB + Redis + JWT, đã test end-to-end). 3 service còn lại (AI Advisory, Marketplace, Notification) vẫn là skeleton rỗng. **Kiến trúc gateway đã đổi từ Nginx sang API Gateway (.NET YARP)** — project `HappyFarmer.ApiGateway` đã nối vào luồng dev: frontend gọi qua Gateway (`:5200`), Gateway tự verify JWT tập trung (JWKS từ Auth Service) rồi forward xuống Auth/Market Price Service kèm header danh tính (`X-User-Id`/`X-User-Role`/`X-User-Phone`). **2 service phía sau đã migrate xong**: không còn tự verify JWT nữa, chỉ tin header Gateway gắn (`AddTrustedHeaderAuthentication`, xem mục Lưu ý kỹ thuật). Frontend (`frontend/`) đã code xong đăng ký/đăng nhập/xem giá nông sản (React + Tailwind + shadcn/ui), đã test end-to-end qua API Gateway với backend thật (browser thật, Playwright). Repo đã push lên GitHub public: `https://github.com/Flakes23/HappyFarmer`.

## Backend solution — `HappyFarmer_Backend/HappyFarmer_Backend.slnx`

| Service (xem docs tương ứng) | Project path | Trạng thái |
|---|---|---|
| [API Gateway](docs/architecture/01-overview.md#3-có-cần-api-gatewaybff-không) (.NET YARP) | `src/Gateway/HappyFarmer.ApiGateway/` | Skeleton |
...
- Gọi qua API Gateway (`:5200`, xem `src/Gateway/HappyFarmer.ApiGateway/`) qua 1 biến duy nhất `VITE_API_GATEWAY_URL` trong `.env` (copy từ `.env.example`) — Gateway route `/api/auth/*` và `/api/market-price/*` xuống đúng service. Chạy dev cần bật cả 3 process: Auth Service (`:5242`), Market Price Service (`:5262`), API Gateway (`:5200`) — thiếu Gateway thì frontend không gọi được API nào (đã bỏ hẳn URL service lẻ khỏi frontend).
