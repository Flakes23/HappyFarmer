...
- Đã code xong: đăng ký, đăng nhập (+ refresh-token tự động khi access token hết hạn), xem giá nông sản (filter theo sản phẩm/khu vực, top biến động giá, biểu đồ lịch sử giá), **Chợ nông sản** (`/marketplace`: đăng bán kèm ảnh Cloudinary + sửa/đóng tin (Farmer), đăng yêu cầu mua (Buyer), liên hệ tin đăng, xem liên hệ/tin của tôi — route guard theo role qua `RequireAuth`).
